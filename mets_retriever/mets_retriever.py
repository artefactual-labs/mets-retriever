...
class METSRetriever:
    ...
    def download_mets_file(self, uuid):
        ...
        # Verify METS file was written correctly.
        expected_path = os.path.join(self.output_directory, f"METS.{uuid}.xml")
    ...
    def download_all_mets_files(self):
        ...
        aips = [aip for aip in am.aips() if aip["status"] == "UPLOADED" and aip["replicated_package"] is None]
        for aip in aips:
            aip_uuid = aip["uuid"]
            ...
            if not aip_in_database:
                try:
                    ...
                    logger.error(f"Unable to download METS file for AIP with UUID {aip_uuid}: {err}")
